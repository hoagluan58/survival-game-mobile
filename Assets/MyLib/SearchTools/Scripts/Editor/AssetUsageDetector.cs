    ...
    public class SceneObjectReferences
    {
        ...
        public void DrawOnGUI()
        {
            ...
            for (int i = 0; i < references.Count; i++)
            {
                ...
                GUI.color = Selection.activeObject == references[i].reference ? Color.green : Color.yellow;
...
public class AssetUsageDetector : EditorWindow
{
    ...
    private bool   searchSystemScript       = false; // Scenes currently open in Hierarchy view
    ...
    // Add menu named "Find Asset Usage" to the Tools menu
    [MenuItem("Find/Find Asset Usage", false, 1)]
    ...
    void OnGUI()
    {
        ...
        else if (currentPhase == Phase.Setup)
        {
            ...
            if (searchSystemScript)
            {
                assetToSearch = EditorGUILayout.ObjectField("Component: ", assetToSearch, typeof(UnityEngine.Component), true);
            }
            else
            {
            ...
            searchSystemScript = EditorGUILayout.ToggleLeft("Find components similar to this.", searchSystemScript);

            if (assetToSearch && searchSystemScript)
            {
                Type type = assetToSearch.GetType();
    ...
    private void ExecuteQuery()
    {
        ...
        if (assetType != AssetType.MultipleSprite)
        {
            ...
            else if (assetToSearch is Component && searchSystemScript)
                assetType = AssetType.SystemScript;
            else
                assetType = AssetType.Other;
    ...
    private void CheckGameObjectForAssetRecursive(GameObject go)
    {
        ...
        if (assetType == AssetType.GameObject && PrefabUtility.GetCorrespondingObjectFromSource(go) == assetToSearch)
        {
        ...
        else
        {
            ...
            for (int i = 0; i < components.Length; i++)
            {
                ...
                else if (assetType == AssetType.SystemScript && component is Component)
                {
                    // If selected asset is a script, check if this component is an instance of it

                    if (component.GetType() == assetToSearch.GetType())
                    {
                        currentSceneReferences.AddReference(component);
