...
namespace SearchTools
{
    public class LinkAnalyzer : System.IDisposable
    {
        public struct AssetUniqueID
        {
            ...
            public static AssetUniqueID none
            {
                get
                {
        ...
        public static AssetUniqueID ConvertObjectToUniqueID(Object obj)
        {
            ...
            if (hasFileID)
            {
                ...
                var fileID     = Unsupported.GetLocalIdentifierInFileForPersistentObject(obj);
                result.fileID = (int)fileID;
            }
            return result;
        }
            ...
            if (IsAsset(uniqueID))
            {
                ...
                if (uniqueID.fileID != 0)
                {
                    result = CustomGUIDetail.LoadAllAssetsAtPath(assetPath)
                                            .Where(x => (int)Unsupported.GetLocalIdentifierInFileForPersistentObject((Object)x) == uniqueID.fileID)
                                            .FirstOrDefault();
                }
                else
                {
