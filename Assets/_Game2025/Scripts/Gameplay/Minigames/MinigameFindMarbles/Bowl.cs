...
namespace SquidGame.LandScape.MinigameFindMarbles
{
    public class Bowl : MonoBehaviour
    {
        ...
        private void Awake()
        {
            _startPosition = _bowlModel.transform.localPosition;
            _startRotation = _bowlModel.transform.eulerAngles;
        }

        private void OnEnable()
        {
            _bowlModel.transform.localPosition = _startPosition;
            _bowlModel.transform.eulerAngles = _startRotation;
        }

        public void SetCorrectBowl(bool value) => _isCorrectBowl = value;

        public void SetClickable(bool isClickable) => _isClickable = isClickable;

        public void AddMarbles(Transform marbles)
        {
            _marbles = marbles;
            _marbles.SetParent(this.transform);
            _marbles.transform.localPosition = new Vector3(0, 0, 0.1f);
        }
    ...
    if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began)
    {
        ...
        public void UpsideDown()
        {
            ...
            var point1 = new Vector3(0, 0.25f, -0.2f);
            var point2 = new Vector3(0, 0, -0.05f);
            _bowlModel.DOLocalPath(new[] { point1, point2 }, 0.5f, PathType.CatmullRom);
        }
    }
}
