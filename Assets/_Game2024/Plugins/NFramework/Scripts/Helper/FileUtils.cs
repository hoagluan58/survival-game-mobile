...
namespace NFramework
{
    public static class FileUtils
    {
        ...
        public static T LoadFirstAssetWithName<T>(string assetName, string folderPath, string overrideFilter = null) where T : UnityEngine.Object
        {
            if (string.IsNullOrEmpty(assetName) || string.IsNullOrEmpty(folderPath))
                return null;

            var filter = overrideFilter != null ? overrideFilter : $"t:{typeof(T).Name}";
            var pathDic = PathUtils.GetAssetsPathDictionary(filter, new[] { folderPath });
            if (pathDic.ContainsKey(assetName))
                return UnityEditor.AssetDatabase.LoadAssetAtPath<T>(pathDic[assetName]);

            Debug.LogError($"Cannot find asset with name: {assetName} in folder: {folderPath}");
            return null;
        }
#endif
    }
